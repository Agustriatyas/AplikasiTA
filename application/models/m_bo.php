	...
	class M_bo extends CI_Model{
		...
		function get_data_entry(){
			$query = $this->db->query("SELECT
					...
					WHERE c_kasubid = 1 ORDER BY id_perm DESC");
    	return $query->result();  
		}
		...
		function get_data_pembuatan_izin(){
			$query = $this->db->query("SELECT
					...
					INNER JOIN tb_pemohon AS B ON B.id_pemohon = A.id_pemohon
					INNER JOIN tb_perusahaan AS C ON C.id_perusahaan = A.id_perusahaan
					WHERE c_kabid = 1 ORDER BY id_perm DESC");
    	return $query->result();  
		}
		...
		function update_isdisable($id_perm){
			$this->db->query("UPDATE tb_permohonan SET isdisable = 1 WHERE id_perm = $id_perm");
		}
		function update_isdisable_tinjauan($id_perm){
			$this->db->query("UPDATE tb_dataperizinan SET isdisable = 1 WHERE id_perm = $id_perm");
		}
		...
		function edit_data_tinjauan($id_perm, $data){
			$this->db->where('id_perm', $id_perm);
			$this->db->update('tb_dataperizinan', $data);
		}
		...
		function get_data_entry_tinjauan(){
			$query = $this->db->query("SELECT
				...
				INNER JOIN tb_pemohon AS B ON B.id_pemohon = A.id_pemohon
				INNER JOIN tb_perusahaan AS C ON C.id_perusahaan = A.id_perusahaan
				INNER JOIN tb_dataperizinan AS D ON D.id_perm = A.id_perm
		...
		function update_isbap($id_perm){
			$query = $this->db->query("UPDATE tb_permohonan SET isbap = '1' WHERE id_perm = $id_perm");
		}
