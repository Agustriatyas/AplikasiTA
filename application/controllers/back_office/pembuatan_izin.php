<?php 
	class Pembuatan_izin extends CI_Controller{
		/**
		 ...
		 */
		function index(){
			$entry_data = $this->m_bo->get_data_pembuatan_izin();
			$this->load->view('back_office/v_pembuatan_izin',array('entry_data'=>$entry_data));
		}
		/**
		 * Fungsi ini digunakan untuk mencetak SK IM
		*/

		function cetak_sk($id_perm = NULL){
			$this->load->library('PHPWord');
  			$document = $this->phpword->loadTemplate('application/docs/temp/SK.docx');

			$query = $this->db->query("SELECT A.id_perm,  A.peruntukan, A.id_pemohon, A.id_lokasi, A.id_tinjauan, A.tgl_permohonan,
					B.pemilik_tanah_v, B.thn_lahir_v, B.pekerjaan_v, B.kewarganegaraan_v, B.luas_bangunan_tutupan_v, B.no_hak_milik_v, B.gsb_v, B.gsp_v, B.gss_v, B.gsr_v, B.jenis_bangunan_v, B.status_tanah_v,
				...
				foreach ($query->result() as $row){
					$tgl_permohonan = $row->tgl_permohonan;
		   			...
		   			$status_tanah_v = $row->status_tanah_v;
				...
				$document->setValue('tgl_permohonan',$tgl_permohonan_format);
				$document->setValue('thn',date('Y'));
				$document->setValue('tgl_tinjauan',$tgl_tinjauan_format);
				$document->setValue('status_tanah',$status_tanah_v);
