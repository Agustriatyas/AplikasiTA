			...
			if (row){
				$.messager.confirm('Confirm','Anda yakin akan menghapus data ini?',function(r){
					if (r){
						...
						url : site+'index.php/front_office/pemohon/delete/'+r,
						// $.post(url,{id_barang:row.id_barang},function(result){
						$.post(url,{id_perm:row.id_perm},function(result){
						//$.post('data_master/barang/remove_barang.php',{id_barang:row.id_barang},function(result){
						$.post('FolderCI/index.php/front_office/pemohon/delete/',{id_perm:row.id_perm},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
