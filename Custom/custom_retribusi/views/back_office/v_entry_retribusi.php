	...
	<body>
		...
		<form name="frmRetribusi" class="form-horizontal" method="POST" role="form" data-toggle="validator" enctype="multipart/form-data" action="<?php echo base_url();?>index.php/back_office/entry_retribusi/insert">
			...
			<table id="idTblRetribusi">
				...
				<tr>
					...
					<td>
						<select name="selLantai0" onChange="setValueNilai(this.value,0);">
					...
					<td>
						<input size="5" type="text" id="idLantai0" name="txtNilaiLantai0" disabled>
					</td>
					<td>
						<select name="selJenisBangunan0" onChange="setJenisBangunan(this.value,0);">
							<option value="">--select--</option>
					...
					<td>
						<input size="5" type="text" id="idPersentaseJB0" name="txtPersentaseJB0" disabled> %
					</td>
					<td>
			...
			<table>
				<tr>
					<th colspan="6" align="right" width="81.45%">Total Retribusi</th>
		...
		<script>
			function addRow(RowCount){
				document.frmRetribusi.hdnIntRowCount.value = parseInt(RowCount) + 1;
				...
				newTD.innerHTML = "<select name='selLantai"+RowCount+"' onChange='setValueNilai(this.value,"+RowCount+");'><option value=''>--select--</option><option value='lt01'>Lantai 1</option><option value='lt02'>Lantai 2</option><option value='lt03'>Lantai 3</option><option value='lt04'>Lantai 4</option><option value='lt05'>Lantai 5</option></select>";
				var newTD = newTR.insertCell(2);
				newTD.innerHTML = "<input type='text'  size='5' name='txtLantai"+RowCount+"' id='idLantai"+RowCount+"' disabled>";
				newTD.align = "center";
				var newTD = newTR.insertCell(3);
				newTD.innerHTML = "<select name='selJenisBangunan"+RowCount+"' onChange='setJenisBangunan(this.value,"+RowCount+");'><option value=''>--select--</option><option value='Perusahaan'>Perusahaan</option><option value='Rumah Tinggal'>Rumah Tinggal</option><option value='Sosial Umum'>Sosial Umum</option></select>";
				var newTD = newTR.insertCell(4);
				newTD.innerHTML = "<input type='text' size='5' id='idPersentaseJB"+RowCount+"' name='txtPersentaseJB"+RowCount+"' disabled> %";
				var newTD = newTR.insertCell(5);
				newTD.innerHTML = "<input type='text' id='idLuas"+RowCount+"' name='txtLuas"+RowCount+"' onkeyup='getTransactionRet(0)'>";
			...
			// set nilai
			function setValueNilai(data,idx){
				if(data == 'lt01'){
			...
			// transaksi
			function getTransactionRet(idx){
				// begin getidperm
			...
			// jenis bangunan
			function setJenisBangunan(data,idx){
				if(data == 'Perusahaan'){
